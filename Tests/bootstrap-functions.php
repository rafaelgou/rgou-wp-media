...
function init_test_suite( $test_suite = 'Unit' ) {
	...
	require_once RGOU_WP_MEDIA_PLUGIN_ROOT . '/vendor/autoload.php';
	...
	require_once RGOU_WP_MEDIA_PLUGIN_ROOT . '/vendor/antecedent/patchwork/Patchwork.php';
}
...
function check_readiness() {
	if ( version_compare( phpversion(), '5.6.0', '<' ) ) {
		trigger_error( 'Rgou\WPMedia Unit Tests require PHP 5.6 or higher.', E_USER_ERROR ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error -- Valid use case for our testing suite.
	}
...
function init_constants( $test_suite_folder ) {
	define( 'RGOU_WP_MEDIA_PLUGIN_ROOT', dirname( __DIR__ ) . DIRECTORY_SEPARATOR );
	define( 'RGOU_WP_MEDIA_PLUGIN_TESTS_ROOT', __DIR__ . DIRECTORY_SEPARATOR . $test_suite_folder . DIRECTORY_SEPARATOR );

	if ( 'Unit' === $test_suite_folder && ! defined( 'ABSPATH' ) ) {
		define( 'ABSPATH', RGOU_WP_MEDIA_PLUGIN_ROOT );
	}
}
